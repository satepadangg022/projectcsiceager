...
@section('title')
Landing Page - Eager Loading
@endsection

@section('content')

{{-- Loading Type Badge --}}
<div class="loading-badge" style="background:rgba(51,154,240,.12);border-color:rgba(51,154,240,.38);color:#74c0fc;">EAGER LOADING</div>

{{-- Performance Stats Badge --}}
<div class="perf-badge">
  <div class="perf-badge-title">Performance</div>
  <div>Total Images: <span id="total-images">0</span></div>
  <div>Load Time: <span id="load-time">—</span></div>
  <div class="perf-badge-note">loading="eager" + fetchpriority</div>
</div>
...
<link rel="preload" as="image" href="{{ asset('img/gambar3.png') }}">

<style>
  .perf-badge {
    position: fixed; bottom: 20px; right: 20px;
    background: rgba(7,22,40,.95); border: 1px solid rgba(51,154,240,.3);
    color: var(--white); padding: 14px 18px; border-radius: 10px;
    font-family: 'Space Grotesk', sans-serif; font-size: .75rem;
    z-index: 1000; backdrop-filter: blur(12px); min-width: 220px;
    line-height: 1.8;
  }
  .perf-badge-title { font-weight: 700; color: #74c0fc; margin-bottom: 4px; }
  .perf-badge-note  { font-size: .68rem; color: var(--muted); margin-top: 6px; }
  #load-time        { color: #74c0fc; font-weight: 700; }
</style>
    ...
    <div class="profil-inner">
      ...
      <div class="profil-img-col reveal">
        <div class="profil-img-wrap">
          ...
          {{-- fetchpriority="high" karena gambar ini above-the-fold di banyak layar --}}
          <img src="{{ asset('img/gambar2.png') }}" loading="eager" fetchpriority="high" alt="Robot hand with shield" class="img-float">
        </div>
      </div>
      ...
      <div class="profil-content reveal delay-1">
        ...
        <div class="services-list">
          <div class="profile-item">
            ...
            <span class="service-text">Pemberian Peringatan Terkait Keamanan Siber</span>
          </div>
          <div class="profile-item">
    ...
    <div class="vm-layout">
      ...
      <div class="vm-img-col reveal delay-2">
        <div class="vm-img-wrap">
          ...
          <img src="{{ url('img/visimisi.png') }}" loading="eager" alt="Collaboration illustration" class="img-float-vm">
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="wrap">
    <div class="section-header-row reveal">
      <div>
        ...
        <h2 class="h2 display">Berita <span class="grad">Keamanan Siber</span></h2>
      </div>
      <a href="#" class="read-more-link">
        Semua Berita <iconify-icon icon="ph:arrow-right-bold"></iconify-icon>
      </a>
    </div>
    ...
    <div class="berita-grid">
      ...
      <a href="{{ route('detail.berita', $b->id) }}" class="news-card reveal">
        <div class="news-img">
          @if(!empty($b->img))
          @env('local')
            <img src="{{ url('')}}/berita/{{($b->img) }}" loading="eager" alt="{{ $b->title }}">
          @endenv
          @production
          <img src="{{ url('')}}/public/berita/{{($b->img) }}" loading="eager" alt="{{ $b->title }}">
          @endproduction
          @else
        ...
        <div class="carousel-track" id="info-track">
          ...
          <div class="info-slide">
            <div class="info-card" data-bs-toggle="modal" data-bs-target="#infografisModal{{ $info->id }}">
              @env('local')
              <img src="{{ url('')}}/infografis/{{$info->img}}" loading="eager" alt="{{ $info->name }}" class="info-img">
              @endenv
              @production
              <img src="{{ url('')}}/public/infografis/{{$info->img}}" loading="eager" alt="{{ $info->name }}" class="info-img">
              @endproduction
              <div class="info-foot">
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-body text-center p-4">
        ...
        @env('local')
        <img src="{{ url('')}}/infografis/{{($info->img) }}" loading="eager" class="img-fluid" alt="{{ $info->name }}">
        @endenv
        @production
        <img src="{{ url('')}}/public/infografis/{{($info->img) }}" loading="eager" class="img-fluid" alt="{{ $info->name }}">
        @endproduction
      </div>
    </div>
  </div>
          ...
          <div class="carousel-inner">
            @for($i = 1; $i <= 5; $i++)
              <div class="carousel-item @if($i == 1) active @endif">
                ...
                <img src="{{ asset('infografis/' . $i . '.png') }}" loading="eager" class="img-fluid modal-img" alt="Infografis {{ $i }}">
              </div>
            @endfor
          </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    ...
    console.log('🔵 EAGER LOADING COMPLETED');
    console.log('Teknik: loading="eager" + fetchpriority="high" pada gambar critical');
